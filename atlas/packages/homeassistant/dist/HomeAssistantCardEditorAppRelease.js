...
export function createHomeAssistantCardEditorAppReleaseReadiness() {
    const summary = checks.reduce((accumulator, check) => {
        if (check.status === "ready")
            return { ...accumulator, ready: accumulator.ready + 1 };
        if (check.status === "in-progress")
            return { ...accumulator, inProgress: accumulator.inProgress + 1 };
