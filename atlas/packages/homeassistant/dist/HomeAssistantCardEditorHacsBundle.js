import { summarizeHomeAssistantCardImport, } from "./HomeAssistantCardConfiguration";
import { createHomeAssistantCardEditorScriptExport } from "./HomeAssistantCardEditorPlan";
export function createHomeAssistantCardEditorHacsBundle(cardPackage) {
    ...
    return {
        ...
        files: [
            ...
            },
            {
                ...
                content: createHomeAssistantCardEditorBundleReadme(script.defaultConfig.title, script.resourcePath, defaultConfig, cardPackage.manifest.languages, cardPackage.manifest.fallbackLanguages),
            },
            {
...
export function inspectHomeAssistantCardEditorHacsBundleArchive(content) {
    const requiredFiles = [
        "hacs.json",
        "README.md",
...
export function readHomeAssistantCardEditorHacsBundleArchivePackage(content) {
    ...
    try {
        ...
        const scriptReadiness = readHacsBundleArchiveScriptReadiness(content, entries, hacsMetadata.filename, summary.script?.customElementName);
        ...
        const readmeReadiness = readHacsBundleArchiveReadmeReadiness(content, entries, summary.script?.resourcePath, summary.script?.cardType);
...
export function createHomeAssistantCardEditorHacsBundleArchive(input) {
...
export function formatHomeAssistantCardEditorHacsBundlePackageReadReviewLines(packageRead) {
...
export function formatHomeAssistantCardEditorHacsBundleReadinessOverviewLines(packageRead) {
...
export function formatHomeAssistantCardEditorHacsBundleReadinessGroupLines(packageRead) {
    const overview = createHomeAssistantCardEditorHacsBundleReadinessOverview(packageRead);
...
export function formatHomeAssistantCardEditorHacsBundleReadinessAttentionLines(packageRead) {
    const overview = createHomeAssistantCardEditorHacsBundleReadinessOverview(packageRead);
    return [
...
export function formatHomeAssistantCardEditorHacsBundlePackageReadinessReviewLines(packageRead) {
    return [
...
export function createHomeAssistantCardEditorHacsBundleReadinessReport(packageRead) {
    const checks = [];
    const add = (code, status, label, detail) => {
    ...
    const hasRequiredFile = (path) => !inspection.missingFiles.includes(path);
    ...
    const checkStatuses = new Set(["pass", "fail", "pending"]);
...
export function createHomeAssistantCardEditorHacsBundleReadinessOverview(packageRead) {
    const report = createHomeAssistantCardEditorHacsBundleReadinessReport(packageRead);
...
function getHacsBundleReadinessStatus(ready, failed) {
    return ready ? "ready" : failed > 0 ? "blocked" : "pending";
}
function readHacsBundleArchiveMetadata(content, entry, scriptFiles) {
...
function getHacsBundleReadinessCheckGroupId(code) {
    ...
    if (code.startsWith("hacs-"))
        return "manifest";
    if (code.startsWith("package-") || code.startsWith("atlas-package"))
        return "package";
    if (code.startsWith("locale-") || code.startsWith("declared-locales"))
        return "locale";
    if (code.startsWith("script-"))
        return "script";
    if (code.startsWith("example-"))
        return "example";
    if (code.startsWith("readme-"))
        return "readme";
    return "import";
}
function readHacsBundleArchiveScriptReadiness(content, entries, scriptFilename, expectedCustomElementName) {
...
function readHacsBundleArchiveExampleReadiness(content, entries, expectedType) {
...
function readHacsBundleArchiveReadmeReadiness(content, entries, expectedResourcePath, expectedCardType) {
...
function readHacsBundleArchiveLocaleReadiness(content, entries, packageContent, inspection) {
...
function createHomeAssistantCardEditorBundleReadme(cardName, resourcePath, defaultConfig, languages, fallbackLanguages) {
    return [
        `# ${cardName}`,
        "",
...
function readZipCentralDirectoryEntries(content) {
...
function readStoredZipEntryText(content, entry) {
...
function readZipEndOfCentralDirectory(content) {
...
function normalizeZipCompressionMethod(value) {
    if (value === 0)
        return "store";
    if (value === 8)
        return "deflate";
...
function createStoredZipArchive(files) {
    const chunks = [];
    ...
    chunks.push(createZipEndOfCentralDirectoryRecord(centralDirectoryRecords.length, centralDirectorySize, centralDirectoryOffset));
    return concatUint8Arrays(chunks);
}
function createZipLocalFileHeader(fileName, crc32, size) {
...
function createZipCentralDirectoryHeader(record) {
...
function createZipEndOfCentralDirectoryRecord(fileCount, centralDirectorySize, centralDirectoryOffset) {
...
function formatInvalidLocaleReviewLine(locale) {
...
function formatScriptReadinessReviewLine(readiness) {
    if (readiness.valid) {
...
function formatExampleReadinessReviewLine(readiness) {
    if (readiness.valid) {
...
function formatReadmeReadinessReviewLine(readiness) {
    if (readiness.valid) {
...
function inspectLocaleArchiveEntry(content, entry, path) {
...
function isSafeHacsBundleArchivePath(path) {
...
function listDuplicateStrings(values) {
    const seen = new Set();
