export function createRuntimePluginInstallPackage(input) {
    return {
...
export function serializeRuntimePluginInstallManifest(plugin) {
...
function readRuntimePluginDescriptor(value) {
    if (!isRecord(value)) {
...
function readInstallPackageFiles(value) {
    if (value === undefined) {
        return [];
    }
...
function readStringList(value) {
    ...
    return value.map(entry => readRequiredString(entry, "Runtime plugin descriptor list entries must be strings."));
}
...
function readOptionalLocalizedText(value) {
    if (value === undefined) {
        return undefined;
    }
    if (!isRecord(value)) {
...
function defaultRuntimePluginReadme(plugin) {
    return [
        `# ${plugin.name}`,
        "",
        `Plugin ID: \`${plugin.id}\``,
